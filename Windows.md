...
After installing python it is recommended to update setuptools before compiling packages.  To update run (example is for python launcher and 3.10+):

    $ py -3.10 -m pip install --upgrade setuptools
...
To build with a specific python version (3.10 through 3.14) or 32 bit vs 64 bit specify that version to the `py` command below.  If you want to build additional versions while keeping all virtual environments setup use a different location for each virtual environment.
...
### With makefile
It is possible to build de-dup with the makefile on windows using a compatible POSIX environment. The following steps have been tested using [msys2][msys2]. Before running make:
1. Install msys2 or other POSIX environment
2. Use the respective console for msys2 (`msys2 msys`)
